...
if __name__ == "__main__":
    ...
    mc.postToChat("Here I come... ready or not!!!")

    time.sleep(2)
    
    #Find the players position
    loc = roundVec3(mc.player.getPos())
    while (True):
        mc.setBlock(loc.x, loc.y, loc.z, 0) #air
        ...
        if (direction == 3):
            newloc.x = loc.x+1
        if (mc.getBlock(newloc.x, newloc.y, newloc.x) == 0):
            loc = newloc
        mc.setBlock(loc.x, loc.y, loc.z, 2)
        time.sleep(0.5)
